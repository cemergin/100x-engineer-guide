...
**Crossplane** — A CNCF-graduated project that extends Kubernetes to manage cloud infrastructure (databases, storage, networking) via CRDs. Enables platform teams to build self-service APIs using Compositions.
...
**Kyverno** — A Kubernetes-native policy engine using YAML policies (no new language). Can validate, mutate, and generate resources. Alternative to OPA/Gatekeeper for admission control.
...
**OPA** (Open Policy Agent) — A CNCF-graduated general-purpose policy engine. Policies are written in Rego, a declarative query language. Used for Kubernetes admission control (via Gatekeeper), Terraform plan validation (via conftest), and API authorization.
...
**Terragrunt** — A thin wrapper around Terraform that eliminates repetition across environments. Manages remote state, dependency ordering, and DRY configurations for multi-environment setups.
...
**SBOM** (Software Bill of Materials) — A formal inventory of all components, libraries, and dependencies in a software artifact. Increasingly required for supply chain security compliance. Generated by tools like Syft and Trivy.

**Sealed Secrets** — A Kubernetes controller (by Bitnami) that decrypts encrypted Secret manifests. Enables committing encrypted secrets to Git safely using asymmetric cryptography.

**Sigstore** — An open-source project for signing, verifying, and protecting software artifacts. Cosign (container signing) and Fulcio (certificate authority) are key components.

**SLSA** (Supply-chain Levels for Software Artifacts) — A security framework with four levels of increasing supply chain integrity. Requires build provenance, authenticated builds, and hermetic reproducibility.

**SOPS** (Secrets OPerationS) — A tool that encrypts values (not keys) in YAML/JSON/ENV files using cloud KMS or age. Enables encrypted secrets in Git with meaningful diffs.
